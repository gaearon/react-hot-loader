...
describe('reconciler', () => {
  describe('Application', () => {
    it('should regenerate internal component', () => {
      const ComponentRoot = createTestDouble(({children}) => <div>{children}</div>, 'root', 'root');


      const Component1 = createTestDouble(({children}) => <b>{children}</b>, 'test', '1');
      const Component2 = createTestDouble(() => <u>REPLACED</u>, 'test', '2');
      const Component3 = createTestDouble(() => <u>NEW ONE</u>, 'somethingElse', '3');

      let currentComponent = Component1;
      const ComponentSwap = (props) => {
        const Component = currentComponent.Component;
        return <blockquote><Component {...props} /></blockquote>;
      }
      ...
      // they are different
      expect(typesAreEqual(Component1.Component, Component2.Component)).toBe(false);
      didUpdate();
      wrapper.setProps({update: 'now'});
      // not react-stand-in merge them together
      expect(typesAreEqual(Component1.Component, Component2.Component)).toBe(true);
      expect(wrapper.find(<Component1.Component/>.type).length).toBe(1);
      ...
      expect(typesAreEqual(Component1.Component, Component3.Component)).toBe(false);
      expect(typesAreEqual(Component2.Component, Component3.Component)).toBe(false);
    });
    ...
    it('should handle function as a child', () => {
      const Component1 = createTestDouble(({children}) => <b>{children(0)}</b>, 'test', '1');
      const Component2 = createTestDouble(({children}) => <u>{children(1)}</u>, 'test', '2');
  ...
  describe('Hydrate', () => {
    ...
    const {children, instance} = hydrate(wrapper.instance());
